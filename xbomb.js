...
let main = async () => {
  ...
  if (err) {
        console.log('MBOMB LOGO ! \n [ CODER ] MOJIB\n[ MADE WITH ] TEAM-TIGER');
        console.dir(err);
        return;
    }
   exec("clear",(maoerr,maoclear) =>{
     
  console.log(maoclear+"\x1b[1;33m"+data+"\n   \x1b[1;30m[ \x1b[1;36m CODED BY \x1b[1;30m]\x1b[1;35m MOJIB\n   \x1b[1;30m[ \x1b[1;34mMAKE WITH \x1b[1;30m]\x1b[1;35m TEAM-TIGER\n\n");
  readline.question(`\x1b[1;32mENTER NUMBER \x1b[1;30m:\x1b[1;33m `,number=>{
    if(number=="mojib" || number=="MOJIB"){
      exec("xdg-open https://github.com/mojib-rsm");
      process.exit(1);
    }else if( number=="team-tiger" || number=="TEAM-TIGER"){
      //
   ...
   for (var i = 0; i < amunt; i++) {
     ...
     request({
       ...
       let url=data["api"][num]["url"].replace("{mobile}",number)
       
       let databody=data["api"][num]["data"].replace("{mobile}",number)
       request({
       url:url,
        method: data["api"][num]["method"],
        headers : data["api"][num]["headers"],
